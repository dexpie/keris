...
def brutal_warning(mode: str = "") -> None:
    ...
    _write(_color("  █                                              █", RED))
    _write(_color("  █   ALAT INI MENJALANKAN UJI AGGRESSIVE.       █", RED))
    _write(_color("  █                                              █", RED))
    _write(_color("  █   HANYA UNTUK TARGET YANG ANDA MILIKI ATAU   █", RED))
    _write(_color("  █   SUDAH MENDAPAT IZIN TERTULIS.               █", RED))
    _write(_color("  █                                              █", RED))
    _write(_color("  █   TANPA IZIN, PENGGUNAAN INI MELANGGAR       █", RED))
    _write(_color("  █   HUKUM (UU ITE) DAN RISIKO PIDANA ADA PADA  █", RED))
    _write(_color("  █   ANDA, BUKAN PADA TOOL INI.                 █", RED))
    _write(_color("  █                                              █", RED))
    _write(_color("  █   JANGAN DIGUNAKAN UNTUK KEJAHATAN SIBER.    █", RED))
    _write(_color("  █                                              █", RED))
    _write(_color("  █   SETUJU? LANJUTKAN. TIDAK? BATALKAN SEKARANG.█", RED))
    _write(_color("  ████████████████████████████████████████████████████", RED))
    _write("")
